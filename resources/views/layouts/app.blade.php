...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/tailwindcss-cdn@3.4.16/tailwindcss.min.js"></script>
    ...
    <style>
        *, *::before, *::after {
        ...
        nav::-webkit-scrollbar-thumb { background: #e2e8f0; border-radius: 999px; }
    ...
    <aside id="sidebar">
        ...
        <div style="padding: 20px 16px; border-bottom: 1px solid #f1f5f9; display:flex; align-items:center; justify-content:space-between;">
            <div style="display:flex; align-items:center; gap:12px;">
                <div style="width:36px; height:36px; background:linear-gradient(135deg,#6366f1,#8b5cf6); border-radius:10px; display:flex; align-items:center; justify-content:center; box-shadow:0 4px 12px rgba(99,102,241,0.35);">
                    <svg style="width:20px;height:20px;color:white;" fill="none" stroke="white" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>
                    </svg>
                </div>
                <div>
                    <p style="font-weight:700; font-size:14px; color:#1e293b; margin:0; line-height:1.2;">Dashboard</p>
                    <p style="font-size:11px; color:#94a3b8; margin:0;">Monitoring Mahasiswa</p>
                </div>
            </div>
            <!-- Close button (mobile only) -->
            <button id="close-btn" onclick="closeSidebar()" style="display:none; background:none; border:none; cursor:pointer; padding:4px; color:#94a3b8; border-radius:8px;" onmouseover="this.style.color='#64748b'" onmouseout="this.style.color='#94a3b8'">
        ...
        <div style="padding:12px 16px; border-top:1px solid #f1f5f9;">
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <div style="flex:1; min-width:0;">
                    <p style="font-size:13px; font-weight:600; color:#1e293b; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ auth()->user()->name }}</p>
                    <p style="font-size:11px; color:#94a3b8; margin:0; text-transform:capitalize;">{{ auth()->user()->role }}</p>
                </div>
                <form method="POST" action="{{ route('logout') }}" style="margin:0;">
                    @csrf
                    <button type="submit" title="Logout" style="background:none; border:none; cursor:pointer; padding:6px; color:#94a3b8; border-radius:8px; display:flex; align-items:center; transition:color 0.15s;" onmouseover="this.style.color='#ef4444'" onmouseout="this.style.color='#94a3b8'">
    ...
    <div id="main-content">
        ...
        <header id="main-header">
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <button id="hamburger" onclick="openSidebar()" style="display:none; background:none; border:none; cursor:pointer; padding:6px; color:#64748b; border-radius:8px; align-items:center;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='none'">
            ...
            <span style="font-size:12px; color:#94a3b8; white-space:nowrap;">{{ now()->translatedFormat('d F Y') }}</span>
        </header>

        <!-- Scrollable area -->
        <main id="main-scroll">
            @if(session('success'))
            <div class="alert-enter" style="margin-bottom:16px; background:#f0fdf4; border:1px solid #bbf7d0; color:#166534; padding:12px 16px; border-radius:12px; font-size:13px; display:flex; align-items:center; gap:8px;">
            ...
            @if(session('error'))
            <div class="alert-enter" style="margin-bottom:16px; background:#fff1f2; border:1px solid #fecdd3; color:#9f1239; padding:12px 16px; border-radius:12px; font-size:13px; display:flex; align-items:center; gap:8px;">
...
<script>
    ...
    function initLayout() {
        if (isMobile()) {
            ...
            // Pastikan sidebar tidak tertutup saat resize ke desktop
            document.getElementById('sidebar').classList.remove('open');
            document.getElementById('overlay').style.display = 'none';
            document.getElementById('close-btn').style.display = 'none';
        }
